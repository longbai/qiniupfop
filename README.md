...
## pfop with exist key
```
bin/qiniupfop -b "<bucket>" -k "<key>" -ak "<access key>" -sk "<secret key>" -n "<notify url>" -c "<convert>" -o "[saveas]"
...
##pfop after upload
```
bin/qiniupfop -b "<bucket>" -k "<key>" -ak "<access key>" -sk "<secret key>" -n "<notify url>" -c "<convert>" -o "[saveas]" -f "[file]"
